<?php 
	trait ProductsModel{
		...
		public function getFavoriteProduct($id){
			$conn = Connection::getInstance();
			$query = $conn->query("select * from products where category_id = (select category_id from products where id=$id) and id not like $id");
			return $query->fetchAll();
		}
		//rating
		public function avgRating($id) {
            $conn = Connection::getInstance();
			$query = $conn->query("SELECT ROUND(SUM(star)/count(*),1) as 'avg' FROM rating WHERE product_id = $id");
			$record = $query->fetch();
			return $record->avg != NULL ? $record->avg : 5;
        }	
        public function modelTotalRating($id) {
            $conn = Connection::getInstance();
